...
点击右上角菜单键中的检查更新选项即可<br>
如果希望每次程序启动自动更新请在菜单键-设置中打开自动更新功能
...
### Q：安装完成后，启动命令行窗口，报错
    优先检查网络配置问题，看能否连接到Github，在安装前使用Steam++/UU加速器等进行加速
    如有必要使用科学上网（作者不提供任何科学上网的方法）
...
### Q：串口号怎么填写
    ...
    但是MuMu模拟器的设备使用adb devices无法列出，需要自己填写，操作如下：
    打开你安装火影忍者的那个模拟器，点进设置的设备诊断，往下翻可以找到端口号，格式为5xxx 16xxx
    左侧5xxx代表串口号“emulator-5xxx”
    右侧16xxx代表串口号“127.0.0.1:16xxx”
    其中xxx是动态分配的，请按照自己的情况填写，最好使用127.0.0.1:16xxx的格式填写
    因为u2库连接adb设备的时候似乎找不到emulator-5xxx这种串口，会报错device not online
    雷电模拟器类似，但区别是两种格式的端口号都可以填写


### Q：报错模拟器连接错误
    这种多半是安装路径填写错误导致的，对于MuMu模拟器，填写最顶层的安装目录即可
    如"D:\Program Files (x86)\MuMuPlayer-12.0"
    不需要填写子目录如"D:\Program Files (x86)\MuMuPlayer-12.0\nx_device\12.0"
    或者"D:\Program Files (x86)\MuMuPlayer-12.0\nx_main"之类的，交给程序自动处理即可
    雷电模拟器填写类似"D:/Program Files (x86)/leidian/LDPlayer9"的路径即可

### Q：模拟器实例索引怎么填写
    按照模拟器多开中给出的顺序填写，比如MuMu的模拟器索引在模拟器列表缩略图的左上角#0，#1，#2，这就是实例索引
    雷电模拟器也在多开的模拟器列表中模拟器信息的最左侧一列可以找到


## III.使用

### Q：有多个账号，换了账号之后为什么不执行任务了？
    每个配置对应一个账号，进度和配置之间互相独立，有几个账号就创建几个配置
...
### Q：为什么我设置体力/铜币购买次数之后，自己买了几次，程序还是按照设定数买导致买多了
...
### Q：出现bug如何反馈？
    ...
    再将"安装目录/log/配置名/日期/Xuan.log"日志连带截图一起压缩打包发送到反馈群/开发者邮箱中

### Q：怎么自动接收更新和bug修复？
    点击程序右上角菜单，在设置中开启"自动更新"即可