...
class WindowCapture:
    ...
    def screencap(self):
        ...
        if not user32.PrintWindow(self.hwnd, self.save_dc.GetSafeHdc(), 0):
        ...
        img = cv2.cvtColor(self.img_buffer, cv2.COLOR_BGRA2BGR)
        height = self.width2height[bmp_info['bmWidth']]
        img = img[bmp_info['bmHeight'] - height:bmp_info['bmHeight'], 0:bmp_info['bmWidth']]
        return img
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            print(f"{(time.time() - start_time) * 1000:.1f}")
