...
class XiaoDuiTuXi(BaseTask):
    ...
    def _execute(self):
        ...
        try:
            ...
            self.logger.debug("进入[我的助战]")
            # 点击我的助战
            self.click_and_wait({
                "type": "ELEMENT",
                "name": "小队突袭-组织助战-我的助战"
            })
            self.logger.debug("领取助战奖励")
            # 点击领取
            if self.click_and_wait({
                "type": "ELEMENT",
                "name": "小队突袭-组织助战-我的助战-领取"
            }):
                self.logger.debug("助战奖励领取成功")

            self.logger.debug("返回[组织助战]")
            # 返回上级小队突袭-组织助战界面
            if not self.esc():
                raise self.StepFailedError("返回[组织助战]失败")

            self.logger.debug("选中四倍奖励")
            if not self.detect_and_wait({
                "type": "ELEMENT",
            ...
            if self.pass_secondary_password():
                self.logger.debug("再次出站")
                # 返回True说明检测到了二级密码的弹窗，则需要再次点击出战
                self.click_and_wait({
                    "type": "ELEMENT",
                    "name": "小队突袭-组织助战-出战"
                }, wait_time=4)
            self.logger.debug("进入自动战斗")
            # 使用连点器过小队突袭
            self.auto_clicker([
                (800, 750)
            ],
    ...
    def _handle_sweep_through(self):
        ...
        if self.click_and_wait({
            ...
            self.logger.debug("存在扫荡按钮，有超影并且已经打过一次")
            # 直接开始点击，点到小队突袭界面出现
            self.logger.debug("进入自动战斗")
            # 使用连点器过小队突袭
            self.auto_clicker(
