        ...
        try:
            ...
            while love_sum < 3:
                ...
                if self.click_and_search(
                    ...
                    love_sum += 1
                    self.logger.debug(f"已点赞❤帖子 {love_sum} 个")
                if retry_time > 20:
                    self.logger.warning("点赞帖子三次失败，可能导致活跃度不足100")
                    break

            self.logger.debug("切换到卷轴界面")
            # 切换到卷轴界面+20活跃度
            self.click_and_wait({
                'type': 'COORDINATE',
                'coordinate': [1477, 356]
            ...
            if not self.click_and_wait({
                ...
                if self.click_and_wait({
                    ...
                    self.logger.debug("未自动弹出窗口，点击一键签到")
                else:
                    self.logger.warning("手动点击一键签到失败，可能已经签到过")
            else:
                self.logger.debug("自动弹出窗口，点击立即签到")
            # 签到成功，点击我知道了
            self.click_and_wait({
                'type': "ELEMENT",
                'name': "福利站-我知道了"
            }, wait_time=3)
            self.logger.debug("进入金币助手")
            # 点击[福利站-今日查看金币助手-去完成]
    ...
    def handle_40_activity_reward(self):
        ...
        if self.click_and_wait({
            ...
            if not self.click_and_wait({
                ...
                self.logger.debug("点击[活跃度-40-确定]失败")
                return True
            ...
            while not self.click_and_wait({
                ...
                continue
            self.logger.debug("40活跃度奖励领取完成")
            return True
        self.logger.debug("40活跃度奖励领取失败，活跃度未达到要求")
        return False

    def handle_60_activity_reward(self):
        self.logger.debug("领取60活跃度奖励")
        if self.click_and_wait({
            'type': "ELEMENT",
            'name': "福利站-活跃度任务-60"
        }):
            ...
            if not self.click_and_wait({
                ...
                self.logger.debug("点击[活跃度-60-我知道了]失败")
                return True
            self.logger.debug("60活跃度奖励领取完成")
            return True
        self.logger.debug("60活跃度奖励领取失败，活跃度未达到要求")
        return False

    def handle_100_activity_reward(self):
        self.logger.debug("领取100活跃度奖励")
        if self.click_and_wait({
            'type': "ELEMENT",
            'name': "福利站-活跃度任务-100"
        }):
            ...
            if not self.click_and_wait({
                ...
                self.logger.debug("点击[活跃度-100-确定]失败")
                return True
            # 点击我知道了
            self.click_and_wait({
                'type': "ELEMENT",
            ...
            self.logger.debug("100活跃度奖励领取完成")
            return True
        self.logger.debug("100活跃度奖励领取失败，活跃度未达到要求")
        return False
