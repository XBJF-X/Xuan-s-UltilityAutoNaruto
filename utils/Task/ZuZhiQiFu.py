...
class ZuZhiQiFu(BaseTask):
    ...
    def _execute(self):
        ...
        if not self.click_and_wait(
                {"type": "ELEMENT", "name": "组织祈福-超影免费"},
                wait_time=2,
                auto_raise=False
        ):
            self.logger.info("超影祈福不存在，点击焚香祈福")
            # 点击组织祈福-焚香祈福
            self.click_and_wait({"type": "ELEMENT", "name": "组织祈福-焚香祈福"}, wait_time=2)
            self.logger.info("[焚香祈福]成功")
        else:
            self.logger.info("点击超影祈福")
        # 点掉上限弹窗
        if self.click_and_wait(
                {"type": "ELEMENT", "name": "组织祈福-今日次数已达上限-确定"},
                wait_time=2,
                auto_raise=False
        ):
        ...
        if self.detect_and_wait(
                {"type": "ELEMENT", "name": "组织祈福-恭喜你获得"},
                auto_raise=False
        ):
            self.logger.info("祈福奖励领取成功")
            # 随便点下关掉弹窗
            self.click_and_wait(
                {"type": "COORDINATE", "coordinate": [1525, 45]},
                wait_time=3
            )
        self.logger.info("领取昨日奖励")
        # 点击昨日奖励
        if self.click_and_wait(
                {'type': "ELEMENT", 'name': "组织祈福-昨日奖励"},
                wait_time=2,
                auto_raise=False
        ):
            # 点击所有的领取按钮
            while self.click_and_wait(
                    {'type': "ELEMENT", 'name': "组织祈福-昨日奖励-领取"},
                    wait_time=2,
                    auto_raise=False
            ):
                continue
            # 随便点下关掉弹窗
            self.click_and_wait({"type": "COORDINATE", "coordinate": [1525, 45]})
